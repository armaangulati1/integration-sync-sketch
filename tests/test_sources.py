...
def test_ics_write_then_read(tmp_path):
    ...
    write_ics(path, [
        {
            ...
            "organizer_name": "Dana Rivers", "account_name": "Northgate Health",
        },
    ])
    ...
    assert r.payload["contact_email"] == "dana.rivers@northgate-health.example"
    assert r.payload["occurred_at"] == "2026-03-02T09:00:00+00:00"
    assert r.payload["account_name"] == "Northgate Health"
...
def test_ics_reader_honors_cursor(tmp_path):
    ...
    write_ics(path, [
        {"uid": "e1@sketch", "summary": "SYNTHETIC 1",
         "start": datetime(2026, 3, 2, 9, tzinfo=UTC),
         "organizer_email": "a@northgate-health.example"},
...
def test_mbox_write_then_read(tmp_path):
    ...
    write_mbox(path, [
        {
            "message_id": "<m1@sketch>", "from_email": "sam.okafor@lakeside-labs.example",
            "from_name": "Sam Okafor", "to": "solutions@vendor.invalid",
            "subject": "SYNTHETIC hi", "date": datetime(2026, 3, 2, 9, tzinfo=UTC),
            "body": "SYNTHETIC body", "account_name": "Lakeside Labs",
        },
    ])
    ...
    assert r.payload["contact_email"] == "sam.okafor@lakeside-labs.example"
    ...
    write_crm_export(path, [
        {
            ...
            "contact_name": "Priya Nadel", "account_name": "Riverbend Clinic",
