...
def test_a_contact_becomes_an_addressable_record():
    ...
    assert first["contact_email"] == "dana.rivers@marnovek-health.example"
    assert first["contact_name"] == "Dana Rivers"
    assert first["account_name"] == "Marnovek Health"
...
def test_a_deal_resolves_its_contact_through_the_association_block():
    emails = {
        "701": "dana.rivers@marnovek-health.example",
    ...
    assert records[0].payload["contact_email"] == "dana.rivers@marnovek-health.example"
...
def test_deals_sync_after_contacts_and_project_into_the_deals_table(store):
    ...
    assert deal["contact_email"] == "dana.rivers@marnovek-health.example"
