...
A **sync engine** reads all five and writes each item once into a single CRM database as
...
without a browser, so the UI is layout and nothing else. A test asserts that every metric
tile on the page carries the number the metrics module returned for it, with one stated
exception: the age of the oldest queued item is measured against wall clock at render time,
so the test cannot pin it to a constant. Everything else on that row and in the funnel is
pinned, so a tile cannot quietly start computing something of its own.
...
171 tests, grouped by guarantee: `test_idempotency.py` (3), `test_incremental.py` (4),
...
`test_ops_metrics.py` (33, the dashboard's numbers), `test_dashboard.py` (4, the page
...
26 functions and collects 33 because the age-band boundary test is parametrized over its 8
cases.
...
the module would pass while every panel threw. Three of those four tests skip cleanly if
streamlit is not installed; the compile check runs everywhere. The sync engine therefore
remains usable without a UI framework, and CI installs the `dashboard` extra so all four run
there.
